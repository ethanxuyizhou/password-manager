...
module Action = struct
  type t = 
    | Update_password_entries of string list
  [@@deriving sexp]
end

let apply_action (model : Model.t) (action : Action.t) = 
  match action with
  | Update_password_entries password_entries ->
    { model with password_entries }

let build ((model : Model.t), apply_action) ~send_rpc=
  let refresh_password_entries () = 
    send_rpc (Sentry_rpcs.list_password_entries_v1, { Sentry_rpcs.User_and_password.user = model.user; master_password = model.master_password })
  |> Effect.inject ~on_response:(function
        | Error (_ : Error.t) -> apply_action (Action.Update_password_entries [])
        | Ok password_entries -> apply_action (Action.Update_password_entries password_entries)
      ) in
  let refresh_button = 
    Vdom.Node.button 
