...
let build ((model : Model.t), apply_action) ~send_rpc =
  let login_banner =
    Vdom.Node.div
      [ Vdom.Attr.id "login-banner" ]
      [ Vdom.Node.text "SENTRY LOGIN" ]
  in
  let login_box =
    Vdom.Node.div
  ...
  let password_box =
    Vdom.Node.div
      [
        ...
        Vdom.Attr.style (Css_gen.text_align `Center);
      ]
      [
        Vdom.Node.input
          [
  ...
  let login_or_fail () =
    send_rpc
      ( Sentry_rpcs.list_password_entries_v1,
        {
          Sentry_rpcs.User_and_password.user = model.login_name;
          master_password = model.login_password;
        } )
    |> Effect.inject ~on_response:(function
         | Error (_ : Error.t) ->
             apply_action (Action.Update_login_status false)
         | Ok (_ : string list) ->
             apply_action (Action.Update_login_status true))
  in
  let login_button =
    Vdom.Node.div
  ...
  let error_box =
    ...
    | Some false ->
        [
          Vdom.Node.div
            [ Vdom.Attr.id "login-error" ]
            [
              Vdom.Node.text
                "Incorrect username or master password. Please try again.";
            ];
        ]
  in
  Vdom.Node.div []
    ([ login_banner; login_box; password_box; login_button ] @ error_box)
