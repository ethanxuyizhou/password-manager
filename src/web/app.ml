...
module Model = struct
  type t = { login : Login.Model.t; 
             home : Home.Model.t;
             currently_rendering : All.t } [@@deriving sexp, equal]

  let update_login t login' =
    { t with login = login' }

  let default = { login = Login.Model.default; home = Home.Model.default; currently_rendering = All.Login } 
  ...
  let update model ?home ?login () =
    ...
    let model = Option.value_map login ~default:model ~f:(fun login -> { model with login = login }) in
...
let apply_action ~inject:_ ~schedule_event:_ (model : Model.t) action =
  match action with
  | Action.Login action -> Model.update model ~login:(Login.apply_action model.login action) ()
  | Home action -> Model.update model ~home:(Home.apply_action model.home action) () 
